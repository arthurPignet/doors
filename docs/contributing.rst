...
Thank you for considering contributing to the Doors: Deep dOppler brOadening cRoss Section project!
...
.. code-block:: console
    ...
    $ git clone git@github.com:arthurPignet/doors.git
...
.. _Github repository: https://github.com/arthurPignet/doors
...
instead to automatically run ``isort``, ``black``, ``docformatter``, ``flake8``,
``pylint``, ``mypy`` as well as other standard formatting functionalities.
...
from the root of the ``deep_doppler`` repository. Now all of
...
To contribute to the DOORS project, follow these steps:
...
The Deep representation of DoCross sections with Doppler broadening effect project uses  `pytest <https://docs.pytest.org/>`_
...
The Doors project uses `Sphinx <https://www.sphinx-doc.org/>`_
